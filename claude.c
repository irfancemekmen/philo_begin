...
typedef struct s_data
{
    ...
    pthread_mutex_t *eat_mutex;     // Her filozof için yeme mutex'i
...
void check_philosopher(t_data *data, int id)
{
    ...
    // Eğer filozof aç ve her iki komşusu da yemek yemiyorsa
...
void take_forks(t_philo *philo)
{
    ...
    // Eğer yemek yiyebiliyorsa, izin bayrağı 1 olur
    int can_eat = data->eat_flags[id];

    pthread_mutex_unlock(&data->state_mutex);

    // Yemek yiyemiyorsa, izin gelene kadar bekle
    if (!can_eat)
    {
        while (!data->simulation_stop)
        {
            ...
            // Durum mutex'ini al ve kontrol et
            pthread_mutex_lock(&data->state_mutex);
            can_eat = data->eat_flags[id];
            pthread_mutex_unlock(&data->state_mutex);

            if (can_eat || data->simulation_stop)
                break;
        }
    }
...
int init_simulation(t_data *data)
{
    ...
    // Yeme mutex'leri için bellek ayır
    data->eat_mutex = malloc(sizeof(pthread_mutex_t) * data->philosopher_count);
    if (!data->eat_mutex)
        return 1;

    // Yeme izin bayrakları için bellek ayır
    data->eat_flags = malloc(sizeof(int) * data->philosopher_count);
    if (!data->eat_flags)
        return 1;
